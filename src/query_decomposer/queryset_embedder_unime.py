...
# ─── local imports ────────────────────────────────────────────────
from src.embedder.unime import UniME
...
def _visible_gpu_ids() -> List[int]:
    ...
    if not env.strip():
        return list(range(torch.cuda.device_count()))  # fallback to all
...
class QueryEmbedderUniMe:
    ...
    def run(self) -> None:
        ...
        # You can filter which datasets you want, e.g.:
    ...
    def _process_dataset(self, dname: str) -> None:
        ...
        out_dir = os.path.join(
            bench_dir, self._alias["embeddings_dirname"], "UniME"
        )
    ...
    def _load_questions(self, dname: str):
        ...
        meta = _load_yaml(METADATA_CONFIG)
        # The main QA file for this dataset
        qa_file = os.path.join(
            self._root, self._subpath["datasets"], dname,
            meta["dataset_metadata"][dname]["filename"]
        )
        bench_type = benchmark_name_to_type(BenchmarkName.from_value(dname))
        bench: LabeledBenchmark = parse_benchmark(bench_type, LabelType.COMPONENT, qa_file)
