...
from src.embedder.mme5 import MME5          # ← your mmE5 embedder
...
class QueryEmbedderMME5:
    def __init__(self, batch_size: int = 4, max_len: int = 512):
        ...
        self.meta              = meta         # stash for _load_questions

    # ---------------------------------------------------------------
    def run(self) -> None:
        mp.set_start_method("spawn", force=True)
        # For quick tests you may restrict datasets like:
    ...
    def _process_dataset(self, dname: str) -> None:
        ...
        out_dir = os.path.join(
            bench_dir, self._alias["embeddings_dirname"], "MME5"
        )
    ...
    def _load_questions(self, dname: str):
        ...
        qa_path = os.path.join(
            self._root, self._subpath["datasets"], dname, dm["filename"]
        )
