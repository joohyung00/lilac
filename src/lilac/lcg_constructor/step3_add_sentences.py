...
from src.utils.utils import (
    ...
    artifact_subpath, ensure_artifact_parsed_documents,
)
...
def parse_arguments():
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["all", "extract_text", "split_sentences", "add_to_parsed_documents", "prepare_for_embedding", "embed"],
        default="all",
        help="Mode of operation. 'all' (default) chains extract_text → split_sentences → add_to_parsed_documents. "
             "Use prepare_for_embedding/embed only if you want to embed sentences without running step 7/8.",
    )
...
class SentenceAdder:
    ...
    def __init__(self, args, config):
        ...
        self._text_component_path     = artifact_subpath(config, ds_name, "component_dirname", "text.json")
        self._sentence_component_path = artifact_subpath(config, ds_name, "component_dirname", "sentence.json")
        os.makedirs(os.path.dirname(self._text_component_path), exist_ok=True)

        self._serialization_path      = artifact_subpath(config, ds_name, "embedding_serializations_dirname", "sentence.json")
        os.makedirs(os.path.dirname(self._serialization_path), exist_ok=True)

        self._embeddings_path         = artifact_subpath(config, ds_name, "embeddings_dirname", "sentence.pt")
        self._index_path              = artifact_subpath(config, ds_name, "embeddings_dirname", "sentence.json")
