...
from src.utils.utils import read_yaml, REPO_ROOT, artifact_subpath
...
def parse_arguments():
    ...
    CONFIG_PATH = f"{REPO_ROOT}/config"
    config = read_yaml(os.path.join(CONFIG_PATH, "lcg_constructor/a.yaml"))
...
def main():
    ...
    image_directory = artifact_subpath(config, args.target_data, "image_docs_dirname")
    ...
    for img_file in tqdm(image_filenames):
        ...
        out_subdir = artifact_subpath(config, args.target_data, "dla_results_dirname", basename)
