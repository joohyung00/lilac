...
from src.experiment.utils.multimodal_qa_utils import *
...
def list_f1(predicted, gold):
    predicted_bags = _answer_to_bags(predicted)
    gold_bags = _answer_to_bags(gold)
    f1_per_bag, recall_per_bag, precision_per_bag = _align_bags(predicted_bags[1], gold_bags[1])
...
def _align_bags(predicted: List[Set[str]], gold: List[Set[str]]) -> List[float]:
    ...
    for gold_index, gold_item in enumerate(gold):
        for pred_index, pred_item in enumerate(predicted):
            if _match_numbers_if_present(gold_item, pred_item):
                scores[gold_index, pred_index], recall, precision = _compute_f1(pred_item, gold_item)
                recalls[gold_index, pred_index] = recall
                precisions[gold_index, pred_index] = precision
    row_ind, col_ind = linear_sum_assignment(-scores)

    max_scores = np.zeros([max(len(gold), len(predicted))])
    max_recalls = np.zeros([max(len(gold), len(predicted))])
    max_precisions = np.zeros([max(len(gold), len(predicted))])
    ...
    return max_scores, max_recalls, max_precisions
