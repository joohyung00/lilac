#!/usr/bin/env python3
"""
Run UniME on several GPUs in parallel and merge the result.

Example
-------
python3 step8_embed_unime.py \
...
# ──────────────────────────────────────────────────────────────────────────────
from src.embedder.unime import UniME  # <── our UniME class
...
def main():
    ...
    # 2) split roughly equally for each GPU
    total = len(corpus)
    per_gpu = math.ceil(total / args.num_gpus)

    tmp_dir = tempfile.mkdtemp(prefix="unime_shards_")
    procs: List[mp.Process] = []

    for g in range(args.num_gpus):
        start, end = g * per_gpu, min((g + 1) * per_gpu, total)
        if start >= end:  # If corpus size < num_gpus
            break

        # Each process gets a slice of the corpus
    ...
    # 4) stitch shards into final files; then clean up tmp_dir
    merge_parts(
        tmp_dir,
...
def worker(
    ...
    """
    Each worker loads one replica of UniME on its assigned GPU
    and writes local embeddings / index into:
      tmp_dir/embeddings_{gpu_id}.pt
      tmp_dir/index_{gpu_id}.json
    """
    # 1) isolate the GPU for the child process
    os.environ["CUDA_VISIBLE_DEVICES"] = str(gpu_id)

    # 2) create the model
    unime = UniME(cuda_num=0, show_progress=(gpu_id == 0))
    unime.load_model()
...
def merge_parts(
    ...
    """
    Concatenate all shard files into the final artefacts and do a consistency check.
    """
    ...
    for g in range(parts):
        ...
        with open(os.path.join(tmp_dir, f"index_{g}.json"), "r", encoding="utf-8") as f:
            local_map = json.load(f)

        # shift index keys to global space
        for k, v in local_map.items():
            # local keys are '0'..'N-1'. We offset them by running_offset
    ...
    print(
        f"✅ Merged embeddings: {final_tensor.shape} • index size: {len(global_index)}"
    )
